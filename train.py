...
def train(
    ...
    X_branch_train, X_trunk_train = X_train
    X_branch_test_data, X_trunk_test_data = X_test_data

    X_branch_border = X_branch_border.to(device)
    X_trunk_border = X_trunk_border.to(device)
    ...
    len_X_train_one = (
        torch.tensor(X_trunk_train.size(0), device=device, dtype=torch.int64) // nb_simu
    )
    for epoch in range(len(train_loss["total"]), nb_it_tot):
        ...
        for nb_batch, batch in enumerate(
            ...
            with torch.cuda.stream(stream_border):
                # loss du border
                pred_border = model(X_branch_border, X_trunk_border)
